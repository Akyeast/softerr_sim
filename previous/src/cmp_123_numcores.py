...
def run_exp(cfg):
    ...
    for state in cfg['num_states_list']:
        ...
        logger = Logger({**cfg, 'num_states': state}, filepath="output/cmp123_crit_numcores", log_params=['num_states', 'critical_prob', 'num_tasks', 'task_max_utilization', 'delta'])
...
def exp(tasks, cfg, logger):
    for task_set in tasks:
        ...
        # num_core_ours = 0
        num_core_only_lockstep = 0
        num_core_dynamic_switching = 0
        num_core_ours = 0
        ...
        for s in range(cfg['num_states']):
            ...
            # print(state_ts)
            new_num_core_only_lockstep, _, _ = get_num_core_only_lockstep(state_ts, cfg['delta'], cfg['max_num_core'], cfg['num_tasks'])
            new_num_core_dynamic_switching, _, _ = get_num_core_dynamic_switching(state_ts, cfg['delta'], cfg['max_num_core'], cfg['num_tasks'])
            new_num_core_ours, _, _ = get_num_core_ours(state_ts, cfg['delta'], cfg['max_num_core'], cfg['num_tasks'])
            # new_num_tasks_ours, prms, mapped_task = get_num_tasks_ours_delta(state_ts, cfg['delta'])
            num_core_only_lockstep = max(num_core_only_lockstep, new_num_core_only_lockstep)
            num_core_dynamic_switching = max(num_core_dynamic_switching, new_num_core_dynamic_switching)
            num_core_ours = max(num_core_ours, new_num_core_ours)

        # logger.write('{}'.format(num_core_ours))
        logger.write('{}, {}, {}'.format(num_core_only_lockstep, num_core_dynamic_switching, num_core_ours))
...
def main():
    with open('cfg/cmp123_crit_numcore_cfg.json', 'r') as f:
        cfg = json.load(f)
