...
from core.only_lockstep import get_num_core_ours_delta, get_num_task_schedulable_only_lockstep
from core.dynamic_switching import get_num_core_ours_delta, get_num_task_schedulable_dynamic_switching
from core.ours import get_num_core_ours_delta, get_num_task_schedulable_ours
...
def run_exp(cfg):
    ...
    for state in cfg['num_states_list']:
        ...
        logger = Logger({**cfg, 'num_states': state}, filepath="output/cmp123_crit_delta07", log_params=['num_states', 'critical_prob', 'num_tasks', 'task_max_utilization', 'delta'])
...
def main():
    with open('cfg/cmp123_crit_cfg.json', 'r') as f:
        cfg = json.load(f)
